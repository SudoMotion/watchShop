  ...
  return (
    <div className="flex justify-center items-center gap-1 mt-4">
      ...
      {visiblePages.map((page, index) => {
        const isActiveItem = typeof page === 'number' && activeitems?.includes(page);
        const isCurrentPage = page === Number(currentPage);
        
        let buttonClass = 'h-10 w-10 border rounded';
        
        if (isCurrentPage) {
          buttonClass += ' bg-gray-900 text-white border-gray-900';
        } else if (isActiveItem) {
          buttonClass += ' bg-green-600 text-white border-green-300';
        } else {
          buttonClass += ' text-gray-700 hover:bg-gray-100 border-gray-600';
        }
        
        buttonClass += page === '...' ? ' cursor-default' : ' cursor-pointer';
        ...
        return (
          <button
            ...
            title={isActiveItem ? 'Previously attempted' : ''}
          >
