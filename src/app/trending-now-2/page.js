...
import {getTrendingNow2 } from "@/stores/ProductAPI";

export const dynamic = "force-dynamic";

export default async function BestDealPage() {
  const response = await getTrendingNow2();
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 md:px-0 py-10 md:py-16">
      <h1 className="title font-semibold">{response.page_name}</h1>
      {products.length > 0 ? (
        ...
        <p className="text-sm text-gray-500 mt-6">No best deal products found.</p>
      )}
    </div>
  );
}
